...
namespace Kurogane.Compiler {
	...
	public static class ExpressionGenerator {
		...
		public static Expression<Func<Scope, object>> Generate(Block block, BinderFactory factory, string fileName) {
			...
			var gen = new GlobalGen(fileName, factory);
			Expression expr = gen.Generate(block);
			Contract.Assume(expr.Type == typeof(object));
	...
	public class GlobalGen : StmtGen {
		...
		// ===== ===== ===== ===== ===== ctor ===== ===== ===== ===== =====
		public GlobalGen(string filename, BinderFactory factory) {
			_filename = filename;
			_factory = factory;
		...
		public Expression Generate(Block block) {
			...
			if (Target == null) {
				if (blockExpr.Type != typeof(object))
					return Expression.Convert(blockExpr, typeof(object));
				else
					return blockExpr;
			}
			else {
	...
	public class DefunGen : BlockGen {
	...
	public class LambdaGen : ElemGen {
